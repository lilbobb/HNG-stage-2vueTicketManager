    ...
    <form @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <p v-if="errors.status" class="mt-1 text-sm text-danger-500">{{ errors.status }}</p>
        </div>
...
const validate = () => {
  const titleError = validateTicketTitle(formData.value.title);
  const statusError = validateTicketStatus(formData.value.status);
